...
void write_scalar_type2( FILE * , int );
	...
	for( int i=0 ; i<elems.size() ; i++ )
	{
		...
		for( int j=0 ; j<elems[i].props.size() ; j++ )
		{
			if( elems[i].props[j].prop.is_list )
			{
				...
				write_scalar_type2( fp , elems[i].props[j].prop.count_external );
				fprintf( fp , " " );
				write_scalar_type2( fp , elems[i].props[j].prop.external_type );
...
void write_scalar_type2( FILE *fp , int code )
{
